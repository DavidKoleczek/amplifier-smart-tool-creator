...
from smart_tool_creator.lib import init, skill
from smart_tool_creator.schemas import OptionalAdapter, SmartToolCreatorError
...
def test_optional_adapter_has_real_transport_and_provider_free_cli(tmp_path: Path, adapter: OptionalAdapter) -> None:
    root = tmp_path / "portable-notes"
    result = init("portable-notes", "Summarizes changelogs", directory=root, adapter=adapter)
    ...
    with ZipFile(next((root / "dist").glob("*.whl"))) as archive:
